...
Sources: UCL x DeepMind Deep Learning 2020, Lecture 2 (Neural Networks
Foundations) and Lecture 5 (Optimization for Machine Learning); *Mathematics for
Machine Learning*, Ch. 5 (vector calculus) for the backprop derivations.

All prose, derivations, and worked examples are written for this course.
Figures are individual instructional diagrams extracted from the UCL decks and
...
FIGURES = [
    ...
    None,                                                 # Why go beyond linear
    "/assets/figures/day03/dnn_universal_approx.png",     # Universal approximation
    None,                                                 # Depth vs width
    # Anatomy of a network
    "/assets/figures/day03/dnn_compgraph.png",            # The MLP / computational graph
    None,                                                 # Activation functions
    None,                                                 # Forward pass
    # Backpropagation
    "/assets/figures/day03/dnn_train_objective.png",      # Learning = minimizing a loss
    None,                                                 # Chain rule on a graph
    None,                                                 # Derivation: four backprop equations
    None,                                                 # Vanishing / exploding gradients
    # Optimization
    "/assets/figures/day03/opt_steepest.png",             # Gradient descent = steepest descent
    "/assets/figures/day03/opt_narrow_valley.png",        # The narrow-valley problem
    "/assets/figures/day03/opt_methods.png",              # SGD / momentum / 2nd-order
    None,                                                 # Derivation: momentum (heavy ball)
    # Generalization
    "/assets/figures/day03/dnn_overfitting.png",          # Overfitting & bias-variance
    None,                                                 # Weight decay & dropout
    None,                                                 # Normalization & early stopping
]
    ...
    [
            ...
            [
                (
                    ...
                    [
                        ...
                        "Find $theta^* = arg min_theta L(theta)$ by gradient descent",
                ...
                (
                    "Derivation: The Four Backprop Equations",
                    [
        ...
        (
            ...
            [
                ...
                (
                    "Derivation: Momentum as a Heavy Ball",
                    [
        ...
        (
            ...
            [
                (
                    "Overfitting & the Bias-Variance View",
                    [
...
LECTURE = {
    ...
    "description": "Multilayer perceptrons, backpropagation, and the optimization and regularization that make them learn.",
    "reading": [
    ...
    "intro": (
        "Yesterday we fit linear and logistic models; today we make them deep. A neural network is "
        "nothing more than a *composition* of simple parameterized maps, but composition buys an "
        "enormous amount: the ability to learn features rather than hand-craft them. We will build the "
        "multilayer perceptron from scratch, derive backpropagation as reverse-mode automatic "
        "differentiation, understand why deep gradients vanish or explode, and study the optimization "
        "and regularization tricks that turn a pile of matrix multiplies into a model that generalizes."
    ),
    "sections": [
        {
            ...
            "subsections": [
                {
                    "heading": "Why linear models are not enough",
                    "definition": (
                        "A **linear model** $$f(\\boldsymbol{x}) = \\boldsymbol{w}^\\top\\boldsymbol{x} + b$$ "
                        "splits the input space with a single hyperplane. A **neural network** replaces the "
                        "fixed features by a learned composition "
...
The early layers play the role of $$\\boldsymbol{\\phi}$$ (representation), the last layer plays the role of $$\\boldsymbol{w}$$ (classifier), and gradient descent tunes them together.""",
                ...
                {
                    "heading": "Universal approximation and the role of depth",
                    "definition": (
                        "**Universal approximation theorem.** For any continuous function "
                        "$$f:K\\subset\\mathbb{R}^n\\to\\mathbb{R}$$ on a compact set and any "
                        "$$\\varepsilon>0$$, there is a one-hidden-layer network "
...
There is also a representational story that matches intuition: a deep vision network learns edges, then textures, then object parts, then objects — a **hierarchy** of features in which later concepts reuse earlier ones. Depth is what makes that reuse possible.""",
        ...
        },
        {
            ...
            "subsections": [
                {
                    "heading": "The multilayer perceptron as a computational graph",
                    "definition": (
                        "A **multilayer perceptron (MLP)** with $$L$$ layers computes, for "
                        "$$l=1,\\dots,L$$, "
                        "$$\\boldsymbol{z}^{(l)} = W^{(l)}\\boldsymbol{a}^{(l-1)} + \\boldsymbol{b}^{(l)},"
                        "\\qquad \\boldsymbol{a}^{(l)} = g\\big(\\boldsymbol{z}^{(l)}\\big),$$ "
...
For $$\\boldsymbol{x}=(1,0)^\\top$$: $$\\boldsymbol{z}^{(1)}=(1,1)^\\top$$, $$\\boldsymbol{a}^{(1)}=\\mathrm{ReLU}(1,1)=(1,1)^\\top$$, and $$\\hat y = 1\\cdot1+1\\cdot1=2.$$ For $$\\boldsymbol{x}=(0,0)^\\top$$ the output is $$0$$. The hidden ReLUs have carved the plane into regions on which the network is locally linear.""",
                },
                {
                    ...
                    "definition": (
                        "An **activation function** $$g$$ is the elementwise nonlinearity applied after "
                        "each affine layer. Without it, "
                        "$$W^{(2)}(W^{(1)}\\boldsymbol{x}+\\boldsymbol{b}^{(1)})+\\boldsymbol{b}^{(2)}$$ "
                        "is *still affine*, so the whole network would collapse to a single linear map."
                    ),
                    "body": """The nonlinearity is what makes depth meaningful. The common choices and their trade-offs:

- **Sigmoid** $$\\sigma(z)=\\dfrac{1}{1+e^{-z}}$$, with $$\\sigma'(z)=\\sigma(z)(1-\\sigma(z))\\le \\tfrac14$$. Smooth and probabilistic, but it **saturates**: for large $$|z|$$ the gradient is ~0, which stalls learning in deep stacks.
...
**Why saturation hurts.** Backprop multiplies the activation derivative at every layer (next section). If each factor is $$\\le\\tfrac14$$ (sigmoid), a 10-layer network attenuates the gradient by up to $$4^{-10}\\approx 10^{-6}$$. ReLU's derivative is exactly $$1$$ on the active region, which is precisely why it rescued deep training.""",
                ...
                {
                    "heading": "The forward pass and output heads",
                    "definition": (
                        "The **forward pass** evaluates the network left-to-right to produce a prediction "
                        "$$\\hat{\\boldsymbol{y}}$$, which is then scored against the target by a **loss** "
                        "$$\\ell(\\hat{\\boldsymbol{y}},\\boldsymbol{y})$$."
                    ),
                    "body": """The last layer is shaped by the task:

- **Regression**: linear output, mean-squared-error loss $$\\ell=\\tfrac12\\|\\hat{\\boldsymbol{y}}-\\boldsymbol{y}\\|^2$$.
...
the difference between predicted and true distributions. This is the error signal that backprop will propagate. During the forward pass we **cache** every $$\\boldsymbol{z}^{(l)}$$ and $$\\boldsymbol{a}^{(l)}$$ because the backward pass needs them — trading memory for a huge saving in compute.""",
        ...
        },
        {
            ...
            "subsections": [
                {
                    "heading": "Learning as loss minimization",
                    "definition": (
                        "**Training** chooses parameters to minimize the empirical risk "
...
2. **Backward** — compute $$\\boldsymbol{\\delta}^{(L)},\\dots,\\boldsymbol{\\delta}^{(1)}$$ and read off parameter gradients.""",
                ...
                {
                    "heading": "Derivation: the four equations of backprop",
                    "definition": (
                        "Backpropagation is four equations: an output-layer error, a recursion, and the "
                        "two parameter gradients. We derive each from the chain rule."
                    ),
                    "body": """Write $$g'(\\boldsymbol{z}^{(l)})$$ for the elementwise derivative and $$\\odot$$ for the elementwise (Hadamard) product.
...
These four lines, applied from $$l=L$$ down to $$l=1$$, *are* backpropagation. Note the recurring structure: every gradient is a product of the upstream error and a cached forward value.""",
                },
                {
                    ...
                    "definition": (
                        "Because $$\\boldsymbol{\\delta}^{(l)}$$ is a product of many factors "
                        "$$(W^{(l+1)})^\\top$$ and $$g'(\\boldsymbol{z}^{(l)})$$, its magnitude can shrink "
                        "toward zero (**vanishing**) or blow up (**exploding**) as it propagates back "
                        "through a deep network."
                    ),
                    "body": """Unrolling the recursion, the error at layer $$l$$ contains a product of $$L-l$$ Jacobian-like factors. If a typical factor has norm $$<1$$, the product decays geometrically; if $$>1$$, it grows geometrically.
...
**Exploding.** Common in recurrent nets (Day 5) where the same matrix is applied many times. The standard fix is **gradient clipping**: rescale $$\\nabla L$$ whenever $$\\|\\nabla L\\|$$ exceeds a threshold. Together, good activations + init + normalization + residuals are what make networks with hundreds of layers trainable.""",
                },
            ],
        },
        {
            ...
            "subsections": [
                {
                    "heading": "Gradient descent is steepest descent",
                    "definition": (
...
To decrease $$L$$ fastest for a fixed step length, we minimize $$\\nabla L^\\top\\boldsymbol{d}$$ over unit $$\\boldsymbol{d}$$; Cauchy–Schwarz makes the minimizer $$\\boldsymbol{d}\\propto-\\nabla L$$. The catch is that the linear model is only trustworthy in a small neighborhood — the smoother (less curved) $$L$$ is, the larger a step $$\\eta$$ we can take. Too large and we overshoot or diverge; too small and progress is glacial. The learning rate is the single most important hyperparameter in deep learning.""",
                ...
                {
                    "heading": "Ill-conditioning: the narrow-valley problem",
                    "definition": (
                        "When the loss is **ill-conditioned** — much steeper in some directions than "
                        "others — a single learning rate cannot serve all directions, and gradient "
...
Neural-network losses are badly conditioned, so plain GD is slow. The next methods attack exactly this: momentum averages out the cross-valley oscillation, and adaptive methods give each coordinate its own effective step size.""",
                ...
                {
                    "heading": "SGD, momentum, and adaptive methods",
                    "definition": (
                        "**Stochastic gradient descent (SGD)** replaces the full-data gradient by a "
...
**Adam** keeps running estimates of the first moment $$\\boldsymbol{m}$$ (mean) and second moment $$\\boldsymbol{v}$$ (uncentered variance) of the gradient, then steps with $$\\theta\\leftarrow\\theta-\\eta\\,\\hat{\\boldsymbol{m}}/(\\sqrt{\\hat{\\boldsymbol{v}}}+\\epsilon)$$. Dividing by $$\\sqrt{\\hat{\\boldsymbol{v}}}$$ gives each parameter its own adaptive step, directly addressing ill-conditioning. In practice: AdamW (Adam + decoupled weight decay) with a warmup-then-cosine-decay schedule is a strong default.""",
                ...
                {
                    "heading": "Derivation: momentum as a heavy ball",
                    "definition": (
                        "Momentum can be read as giving the optimizer **inertia**: the velocity is an "
                        "exponentially weighted sum of all past gradients, so consistent directions "
                        "accelerate while oscillations cancel."
                    ),
                    "body": """Unroll the velocity recursion $$\\boldsymbol{v}_{k+1}=\\beta\\boldsymbol{v}_k+\\nabla L(\\theta_k)$$ from $$\\boldsymbol{v}_0=\\mathbf{0}$$:
...
The name "heavy ball" comes from the physical analogy: plain GD is a massless particle that instantly follows the local slope, while momentum is a ball with mass rolling on the loss surface — its inertia carries it smoothly along the valley floor and through small bumps. Nesterov's variant evaluates the gradient at the *look-ahead* point $$\\theta_k-\\eta\\beta\\boldsymbol{v}_k$$ and enjoys a better worst-case convergence rate.""",
        ...
        },
        {
            ...
            "subsections": [
                {
                    "heading": "Overfitting and the bias–variance view",
                    "definition": (
                        "**Generalization** is performance on unseen data. As model capacity grows, "
...
The practical consequence is that **zero training error is not the goal** — low *test* error is. We hold out a **validation set** to estimate test performance and to choose capacity and hyperparameters, keeping a final **test set** untouched for an honest estimate. (Modern over-parameterized networks complicate the classical picture — the "double descent" phenomenon — but the operational advice, regularize and validate, is unchanged.)""",
                ...
                {
                    "heading": "Weight decay, dropout, and data augmentation",
                    "definition": (
                        "**Regularization** is any modification that reduces test error at the possible "
                        "cost of training error. The workhorses are weight penalties, dropout, and data "
                        "augmentation."
                    ),
                    "body": """**Weight decay ($$L_2$$).** Add $$\\frac{\\lambda}{2}\\|\\theta\\|^2$$ to the loss. The gradient gains a term $$\\lambda\\theta$$, so each step shrinks the weights: $$\\theta\\leftarrow(1-\\eta\\lambda)\\theta-\\eta\\nabla L$$. From a Bayesian angle this is a **Gaussian prior** on the weights — the MAP estimate of $$\\theta$$ under $$\\theta\\sim\\mathcal{N}(0,\\tau^2 I)$$ (Day 2's connection between regularization and priors).

**$$L_1$$ penalty** $$\\lambda\\|\\theta\\|_1$$ drives weights exactly to zero, giving sparse, interpretable models (feature selection).
