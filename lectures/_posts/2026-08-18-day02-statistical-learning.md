...
description: >
  The supervised/unsupervised framework, linear regression, PCA, Gaussian mixture models, and support vector machines.
invert_sidebar: true
---
...
Day 1 built the mathematical language; today we use it to design learning algorithms. We start with the **framework** for how data, models, and learning fit together, then work through the four central pillars: **linear regression**, **PCA** for dimensionality reduction, **Gaussian mixture models** for density estimation, and **support vector machines** for classification. Each section connects back to the linear-algebra and probability tools from Day 1.
...
![Toy regression data: salary vs age with a query point at age 60.](/assets/figures/day02/mml_toy_regression.png)
...
![K-fold cross-validation: the dataset is partitioned into $$K$$ chunks; each chunk serves once as validation while the rest train the model.](/assets/figures/day02/mml_cross_validation.png)
...
![Linear regression example: candidate lines, training data, and MLE fit.](/assets/figures/day02/mml_linear_regression.png)
...
&= -\frac{1}{2\sigma^2}\,\big\Vert \Phi\boldsymbol{\theta}-\mathbf{y}\big\Vert _2^2 + \text{const}.
\end{aligned}$$

Maximizing the log-likelihood is therefore **identical** to minimizing the sum of squared residuals $$\Vert \Phi\boldsymbol{\theta}-\mathbf{y}\Vert _2^2$$ — the noise variance $$\sigma^2$$ only rescales the objective and drops out of the argmin. So "least squares" is not a modeling choice we make for convenience; it is *implied* by the Gaussian assumption.

**Solving it.** Using the gradient identity $$\nabla_{\boldsymbol{\theta}}\Vert \Phi\boldsymbol{\theta}-\mathbf{y}\Vert _2^2 = 2\Phi^{\top}(\Phi\boldsymbol{\theta}-\mathbf{y})$$ (Day 1) and setting it to zero gives the **normal equations** and, when $$\Phi^{\top}\Phi$$ is invertible, the closed form:
...
When $$\Phi^{\top}\Phi$$ is near-singular — collinear features, or more parameters than data ($$M>N$$) — OLS has enormous variance: a tiny perturbation of $$\mathbf{y}$$ swings $$\boldsymbol{\theta}^{\star}$$ wildly, because we are dividing by near-zero directions of $$\Phi^{\top}\Phi$$. Ridge regression fixes this by adding an $$\ell_2$$ penalty to the objective.

**Closed form.** Minimize the penalized objective and set the gradient to zero:

$$\begin{aligned}
L(\boldsymbol{\theta}) &= \tfrac12\Vert \Phi\boldsymbol{\theta}-\mathbf{y}\Vert _2^2 + \tfrac{\lambda}{2}\Vert \boldsymbol{\theta}\Vert _2^2, \\
...
&= \underbrace{-\tfrac{1}{2\sigma^2}\Vert \Phi\boldsymbol{\theta}-\mathbf{y}\Vert _2^2}_{\text{log-likelihood}} \;\underbrace{-\tfrac{1}{2\tau^2}\Vert \boldsymbol{\theta}\Vert _2^2}_{\text{log-prior}} + \text{const}.
\end{aligned}$$
...
![PCA finds a lower-dimensional subspace that preserves variance when data are projected.](/assets/figures/day02/mml_pca_lowdim.png)
...
Why can't we just maximize the GMM log-likelihood directly? Because the density is a **sum** inside the log, $$\log\sum_k \pi_k\mathcal{N}(\mathbf{x}_n\mid\boldsymbol{\mu}_k,\boldsymbol{\Sigma}_k)$$, which does not separate into per-component terms — setting the gradient to zero gives coupled equations with no closed form. EM is the standard way around this, and it is far more general than GMMs.
...
&= \underbrace{\mathbb{E}_{q}\!\Big[\log\tfrac{p_{\boldsymbol{\theta}}(\mathbf{x},\mathbf{z})}{q(\mathbf{z})}\Big]}_{\textcolor{teal}{\mathcal{F}(q,\,\boldsymbol{\theta})\ =\ \text{ELBO}}} \;+\; \underbrace{D_{\mathrm{KL}}\!\big(q(\mathbf{z})\,\Vert \,p_{\boldsymbol{\theta}}(\mathbf{z}\mid\mathbf{x})\big)}_{\textcolor{purple}{\ \geq\ 0}} .
\end{aligned}$$
...
## 5. Classification: Logistic Regression and Support Vector Machines

### 5.1 Logistic regression: cross-entropy from a Bernoulli model

> **Logistic regression** models the class probability as a sigmoid of a linear score, $$p(y=1\mid\mathbf{x}) = \sigma(\mathbf{w}^{\top}\mathbf{x}+b)$$ with $$\sigma(z) = 1/(1+e^{-z})$$. Maximum likelihood under this **Bernoulli** model is exactly minimization of the **cross-entropy** (log) loss.
{:.lead}

Regression assumed Gaussian noise and got squared error. Classification assumes a **Bernoulli** label and gets cross-entropy — the same recipe (loss $$= -\log$$ likelihood), a different distribution. This is the second half of "different assumptions $$\Rightarrow$$ different losses".
...
### 5.2 Separating hyperplanes and margins
...
![Two-dimensional classification data with two class labels.](/assets/figures/day02/mml_svm_2d.png)
...
**Distance from a point to a hyperplane.** The vector $$\mathbf{w}$$ is normal to the hyperplane $$\mathbf{w}^{\top}\mathbf{x}+b=0$$. Decompose any point as $$\mathbf{x} = \mathbf{x}_\perp + r\,\tfrac{\mathbf{w}}{\Vert \mathbf{w}\Vert }$$ with $$\mathbf{x}_\perp$$ on the hyperplane. Plugging in and using $$\mathbf{w}^{\top}\mathbf{x}_\perp+b=0$$,

$$\mathbf{w}^{\top}\mathbf{x}+b = r\,\frac{\mathbf{w}^{\top}\mathbf{w}}{\Vert \mathbf{w}\Vert } = r\,\Vert \mathbf{w}\Vert  \quad\Longrightarrow\quad r = \frac{\mathbf{w}^{\top}\mathbf{x}+b}{\Vert \mathbf{w}\Vert },$$

so the **signed distance** is $$(\mathbf{w}^{\top}\mathbf{x}+b)/\Vert \mathbf{w}\Vert $$. We are free to rescale $$(\mathbf{w},b)$$, so adopt the **canonical** normalization $$\min_n \vert \mathbf{w}^{\top}\mathbf{x}_n+b\vert  = 1$$ at the closest points. The distance from the hyperplane to the nearest point is then $$1/\Vert \mathbf{w}\Vert $$, and the full **margin** between the two classes is $$2/\Vert \mathbf{w}\Vert $$.
...
### 5.3 Hard-margin and soft-margin SVMs
...
**Maximizing the margin = minimizing $$\Vert \mathbf{w}\Vert $$.** Maximizing $$2/\Vert \mathbf{w}\Vert $$ is the same as minimizing $$\tfrac12\Vert \mathbf{w}\Vert ^2$$ (monotone, and quadratic hence convex). With the canonical scaling, "every point on the correct side of its margin" becomes $$y_n(\mathbf{w}^{\top}\mathbf{x}_n+b)\geq 1$$. That gives the **hard-margin primal** — a convex quadratic program:

$$\min_{\mathbf{w},b}\ \tfrac12\Vert \mathbf{w}\Vert ^2 \qquad \text{s.t.}\quad y_n(\mathbf{w}^{\top}\mathbf{x}_n+b)\geq 1,\ \forall n.$$
...
$$\min_{\mathbf{w},b,\boldsymbol{\xi}}\ \tfrac12\Vert \mathbf{w}\Vert ^2 + C\sum_n \xi_n \qquad \text{s.t.}\quad y_n(\mathbf{w}^{\top}\mathbf{x}_n+b)\geq 1-\xi_n,\ \xi_n\geq 0.$$
...
$$\min_{\mathbf{w},b}\ \underbrace{\sum_{n}\max\big(0,\,1-y_n(\mathbf{w}^{\top}\mathbf{x}_n+b)\big)}_{\textcolor{teal}{\text{hinge loss}}} + \underbrace{\tfrac{1}{2C}\Vert \mathbf{w}\Vert ^2}_{\textcolor{purple}{\ell_2\text{ regularizer}}}.$$
...
### 5.4 Dual formulation and the kernel trick
...
**Deriving the dual.** Attach multipliers $$\alpha_n\geq 0$$ to the hard-margin constraints and form the Lagrangian:

$$\mathcal{L}(\mathbf{w},b,\boldsymbol{\alpha}) = \tfrac12\Vert \mathbf{w}\Vert ^2 - \sum_n \alpha_n\big[y_n(\mathbf{w}^{\top}\mathbf{x}_n+b)-1\big].$$
